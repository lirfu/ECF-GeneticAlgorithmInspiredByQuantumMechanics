...
void NeuralNetwork::registerParameters(StateP state) {
    ...
    registerParameter(state, "structure", (voidP) (new string("2 3 1")), ECF::STRING,
                      "number of neurons per layer (default: 2 3 1)");
    registerParameter(state, "outFunction", (voidP) new uint(1), ECF::UINT,
                      "activation function of the output layer (default: 1)");
    registerParameter(state, "configFile", (voidP) (new string()), ECF::STRING,
                      "path to file containing the NN structure and weight values");
    registerParameter(state, "dataFile", (voidP) (new string()), ECF::STRING,
                      "path to file containing training data (mandatory)");
    registerParameter(state, "errorWeights", (voidP) (new string()), ECF::STRING,
                      "path to file containing weights for every training data line.");
    registerParameter(state, "errorFunc", (voidP) (new string("RMSE")), ECF::STRING,
                      "tag for error function used in getTotalError (default: RMSE)");
    registerParameter(state, "activations", (voidP) (new string("sig lin")), ECF::STRING,
                      "Activation functions per layer. Input layer has no activation function.");
}
    ...
    for (uint layer = 2; layer <= getNoLayers(); layer++) {
        ...
        for (uint neuron = 1; neuron <= npLayer_[layer - 1]; neuron++) {
            ...
            for (uint w = 1; w < weights.size(); w++)
                sum += weights[w] * Outputs[w - 1]; // Last layers outputs are this layers neuron inputs.
...
void NeuralNetwork::setActivationFunction(AbstractActivationFunction *activationFunction) {
    activationFunctionMapping_.insert(
            std::pair<string, AbstractActivationFunction *>(activationFunction->name, activationFunction));
}
...
void NeuralNetwork::calculateOutputs() {
    ...
    for (size_t i = 0, n = inputData_.size(); i < n; i++) {
        ...
        calculatedOutputs_.push_back(calculateOutput());
    }
}
...
// Calculate rest of the neuron outputs.
        for (uint layer = 2; layer <= getNoLayers(); layer++) {
            ...
            applyActivationFunctionDerivativeToTheLayer(Derivatives.back(),
                                                        layer); // Apply the activation function derivation for stored input sums.
...
// }}
        ...
        for (uint layer = 2; layer <= getNoLayers(); layer++) {
            for (uint neuron = 1; neuron <= npLayer_[layer - 1]; neuron++) {
                ...
                for (uint w = 1; w <
                                 weights.size(); w++) {                                                                                // Error for this neuron (delta)         * Output of the lower layer (or ultimately inputs)
                    weights[w] = weights[w] +
                                 learningRate * Errors[getNoLayers() - layer][neuron - 1] * Outputs[layer - 2][w - 1];
                }
